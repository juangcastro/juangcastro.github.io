# ANEXO B — TABLERO DE SEGUIMIENTO MENSUAL

> 30 minutos, día 1 de cada mes (Paso 6). Copia esta tabla cada mes nuevo.

## MES: ______ / AÑO: ______

### 1. Patrimonio neto
| | Mes pasado | Este mes | Δ | Señal |
|---|---|---|---|---|
| Activos | $ | $ | $ |  |
| Pasivos | $ | $ | $ |  |
| **Patrimonio neto** | $ | $ | $ | ¿↑ por aporte o por mercado? |

### 2. Aportación
| Planeado | Ejecutado | Cumplimiento | Acción si <80% |
|---|---|---|---|
| $ | $ | % |  |

### 3. Asignación real vs objetivo (IPS)
| Estrategia | Objetivo | Real | Desvío | ¿Rebalancear? (>5%) |
|---|---|---|---|---|
| Preservación | % | % |  | ☐ |
| Ingresos | % | % |  | ☐ |
| Crecimiento | % | % |  | ☐ |

### 4. Flujo del patrimonio (la máquina)
| Fuente | Monto del mes | Anualizado |
|---|---|---|
| Dividendos/rentas | $ | $ |
| Intereses | $ | $ |
| Otros | $ | $ |
| **Total flujo** | $ | $ |

### 5. Costos
| Concepto | Monto | % del patrimonio |
|---|---|---|
| Comisiones / fees | $ |  |
| Impuestos | $ |  |
| Asesores | $ |  |
| **Total** | $ | (meta: <1.5%) |

### 6. Decisiones de este mes
- [ ] Rebalanceo ejecutado (si aplica): ______
- [ ] Aportación del mes realizada: ______
- [ ] Nueva posición evaluada contra IPS: ______ (¿cabe? ☐ sí ☐ no)
- [ ] Impuestos del período cubiertos: ______

### 7. Anotaciones / pendientes
```

```

---

## INSTRUCCIONES DE CADENCIA (recordatorio)

- **Día 1 de cada mes**: este tablero (30 min) + aportación automática
- **Última semana del trimestre**: revisión de cartera (1-2 h) con asesor
- **Mes del cumpleaños**: revisión anual completa (IPS, equipo, estructuras, metas)

> Regla: el tablero NO decide por ti — te muestra. Las decisiones las tomas tú
> contra tu política escrita. Si el tablero muestra algo que tu política no cubre,
> actualiza la política (sección 7 del IPS), no el tablero.
